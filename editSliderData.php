...
function storeData($data, $newFile) {
    ...
    // SQL query
    if (!empty($newFile)) {
        $strSQL = "UPDATE slider set title='" . $data['title'] ."',description='" . $data['desc'] . "',image='" . $newFile . "',status=" . $data['status'] . ",updateOn='" . time() . "' where id=" . $data['id'];
    } else {
        $strSQL = "UPDATE slider set title='" . $data['title'] ."',description='" . $data['desc']. "',status=" . $data['status'] . ",updateOn='" . time() . "' where id=" . $data['id'];
    }
//    echo $strSQL;
//    exit();
