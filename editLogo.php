...
function storeData($data, $newFile) {
    ...
    // SQL query
    if (!empty($newFile)) {
        $strSQL = "UPDATE clients set title='" . $data['title'] . "',logo='" . $newFile . "',status=" . $data['status'] . ",updateOn='" . time() . "' where id=" . $data['id'];
    } else {
        $strSQL = "UPDATE clients set title='" . $data['title'] . "',status=" . $data['status'] . ",updateOn='" . time() . "' where id=" . $data['id'];
    }
//    echo $strSQL;
//    exit();
